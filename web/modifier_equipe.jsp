...
<%
    String id =(String) session.getAttribute("id_e");
    ...
    if (!(nom == null) && !(nom.equals("") && !exist)) {
        ResultSet resultSet6 = Select.query("select nom from equipe;");

        while (resultSet6.next()){
           if(resultSet6.getString("nom").equals(nom)){
               //exist deja
               exist =true;
               response.sendRedirect("gestion_equipe.jsp?ed=ok");
               break;
           }
        }
        if(!exist){
    ...
    if(!exist) {
        ...
        response.sendRedirect("gestion_equipe.jsp?ms=ok");
    }
