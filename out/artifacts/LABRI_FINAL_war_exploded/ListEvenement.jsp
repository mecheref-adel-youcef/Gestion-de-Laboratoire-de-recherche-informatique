...
<body class="container">
      ...
      <% session.setAttribute("IdPersonne", "3"); %>
    
        
      <div class="box box-info" style="padding: 2%" >
        <div class="row">
            <div class="col-md-6"> </div>
            <form action="EvenementControllerServlet" method="get">
                      <div class="col-md-6">
                          <div class="form-group">
                            <div class="input-group ">
                          <input type="hidden" name="cmd" value="RECH" />
    ...
    <div class="row" >
      <div class="col-md-12"  >
        ...
        <table class="table" >
          ...
          <tbody>
          <c:forEach var="temp" items="${List_Evenement}">
          <c:url var="viewlink" value="EvenementControllerServlet">
				<c:param name="cmd" value="LOAD" />
				<c:param name="IdEvenement" value="${temp.getIdEvenement()}" />
			</c:url>
			<c:url var="editlink" value="EvenementControllerServlet">
				<c:param name="cmd" value="EDIT" />
				<c:param name="IdEvenement" value="${temp.getIdEvenement()}" />
			</c:url>
			<c:url var="deletelink" value="EvenementControllerServlet">
				<c:param name="cmd" value="DELETE" />
				<c:param name="IdEvenement" value="${temp.getIdEvenement()}" />
			</c:url>
			<c:url var="participelink" value="EvenementControllerServlet">
				<c:param name="cmd" value="PARTICIPE" />
				<c:param name="IdEvenement" value="${temp.getIdEvenement()}" />
			</c:url>
			<c:url var="nonparticipelink" value="EvenementControllerServlet">
				<c:param name="cmd" value="NONPARTICIPE" />
				<c:param name="IdEvenement" value="${temp.getIdEvenement()}" />
			</c:url>
			<c:url var="allparticipants" value="EvenementControllerServlet">
				<c:param name="cmd" value="ALL" />
				<c:param name="IdEvenement" value="${temp.getIdEvenement()}" />
				<c:param name="Titre" value="${temp.getTitre()}" />
			</c:url>
            <form action="">
            <tr>
              <td>
              ...
              <td>
               <a href="${viewlink}">${temp.getTitre()}</a> 
              </td>
              <td>
                  ${temp.getLieu()}
              </td>
              ...
              <td><a href ="${editlink}" class="btn btn-info active"><i class="fa fa-edit"></i> Edit </a></td>
              <td><a href ="${deletelink}" class="btn btn-danger active" onclick="if (!(confirm('are you sure you want to delete this project'))) return false"><i class="fa fa-trash-alt"></i> Delete</a></td>
              
              <td><a href ="${participelink}" class="btn btn-info active" > Participe</a></td>
              <td><a href ="${nonparticipelink}" class="btn btn-info active" > NonParticipe</a></td>
              <td><a href ="${allparticipants}" class="btn btn-info active" > AllParticipants</a></td>
